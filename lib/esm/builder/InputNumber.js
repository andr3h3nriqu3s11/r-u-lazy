...
export function InputNumberBuilder(BaseInputNumber, Wraper) {
    var InputNumber = function (props) {
        ...
        if (isNaN(valueN))
            valueN = undefined;
        return (React.createElement(Wraper, { id: id, label: label, class: labelClass, gridless: gridless, hidden: hidden },
            React.createElement("div", { className: (suffixOutside ? 'p-grid ' : '') + inputClass }, (function () {
                var a = (React.createElement(BaseInputNumber, { tooltip: tooltip, type: type, value: valueN, onChange: !disabled && onChange ? onChange : function () { }, id: id, disabled: disabled, required: required, suffix: suffixOutside ? '' : suffix, prefix: prefix, mode: mode, currency: currency, minFractionDigits: minFractionDigits, maxFractionDigits: maxFractionDigits, locale: locale }));
                if (suffixOutside)
                    return (React.createElement(React.Fragment, null,
                        React.createElement("div", { className: "p-col" }, a),
                        React.createElement("div", { className: "p-col-1" }, suffix !== null && suffix !== void 0 ? suffix : '')));
                return a;
            })())));
    };
    return InputNumber;
